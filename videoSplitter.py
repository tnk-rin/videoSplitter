...
def main():
	...
	parser.add_argument('-y', '--year', type=str, help='Year to be added to the metadata', default='')
	parser.add_argument('-g', '--genre', type=str, help='Genre to be added to the metadata', default='')
	parser.add_argument('-a', '--album', type=str, help='Album name to be added to the metadata', default='')
	parser.add_argument('-f', '--force', help='Forces files to be replaced instead of skipping files that already exist', action='store_true', default=False)
	...
	for i in range(0, len(times) - 1):
		...
		prevStartTime = timestamp
		clipCount += 1
		metaAdder.add_meta(path + '/' + filename[:len(filename)-1] + '.mp3', args.album, i + 1, len(times), args.year, args.genre)
	filename = times[len(times) - 1].split(' ')
	filename.pop(0)
	filename = (' '.join(filename))
	if (os.path.exists(path + '/' + filename[:len(filename)] + '.mp3') and not args.force):
		print('"' + filename[:len(filename)-1] + '"\n\talready exists, skipping to next item\n')
	else:
		timestamp = clip.duration
		sclip = clip.subclip(prevStartTime, timestamp).audio
		sclip.write_audiofile(path + '/' + filename[:len(filename)] + '.mp3', nbytes=2)
		del sclip
		metaAdder.add_meta(path + '/' + filename[:len(filename)] + '.mp3', args.album, len(times), len(times), args.year, args.genre)
	del clip
	print("Completed")
